class UI {
    constructor(scene) {
        ...
        this.pg1Controls = document.querySelectorAll('#controls-pg1 button');
        this.pg2Controls = document.querySelectorAll('#controls-pg2 button');
    ...
    _bindControls() {
        // Funzione helper per legare gli input sia al mouse che al touch (zero lag)
        const bindButton = (id, action) => {
            const btn = document.getElementById(id);
            if (!btn) return;
            // Usa pointerdown per supportare touch e mouse ed evitare delay su mobile
            btn.addEventListener('pointerdown', (e) => {
                e.preventDefault(); // Evita scroll o zoom accidentali da touch
    ...
    updateTurn(currentTurn, bombUsed) {
        ...
        this.pg1Controls.forEach(btn => btn.disabled = (currentTurn !== 1));
        this.pg2Controls.forEach(btn => btn.disabled = (currentTurn !== 2));
        
        if (currentTurn === 1 && bombUsed[1]) this.btnBomb1.disabled = true;
        if (currentTurn === 2 && bombUsed[2]) this.btnBomb2.disabled = true;
    }
    ...
    triggerExplosionFX(explodingId, winnerId) {
        this.pg1Controls.forEach(btn => btn.disabled = true);
        this.pg2Controls.forEach(btn => btn.disabled = true);
        ...
        setTimeout(() => {
            ...
            this.gameOverBanner.style.color = "#000";
